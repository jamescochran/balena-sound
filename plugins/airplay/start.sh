...
while [[ "$(curl --silent --head --output /dev/null --write-out '%{http_code}' --max-time 2000 'http://localhost/audio')" != "200" ]]; do sleep 5; echo "Waiting for audioblock to start..."; done
